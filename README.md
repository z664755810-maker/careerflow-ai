...
④ 进入 AI 分析页，历史表格默认已有 5 条示例分析（含高/中/低匹配度），点「查看」可见四维雷达 + **评分依据（期望薪资 vs 岗位薪资范围）** + 逐维度摘要；选简历+JD 点「开始分析」可生成实时结果（需 `LLM_API_KEY`）；分析详情里点「🎤 用本次分析开始模拟面试」可一键跳到 `/interview` 开聊。`/interview` 模拟面试页默认已有 1 条**已完成的三轮示例会话**（累计均分 82），可选分析开启新面试（作答需 `LLM_API_KEY`）。
